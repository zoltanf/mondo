...
- **Batch:** `item create --batch <file.json|- >` bulk-creates via aliased mutations (~10 items per HTTP call, `--chunk-size` to tune; per-row envelope: partial failure → exit 1). No other command has `--batch` yet — bulk `column set` across items needs a loop, and at ~2.4s/call 50+ items will blow a 2-minute Bash timeout: run it in the background or raise the timeout.
...
- **Empty a column with `mondo column clear`** — it sends the type-correct empty payload. Guessed empties through `column set` (`--value ""`, `'{"labels":[]}'`) fail codec validation.
