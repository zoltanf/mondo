...
- **Resolve ids by name with the cheap shape:** `mondo item list --board X --group G --fields id,name` is the canonical id lookup — `--fields id,name` (or a `-q` touching only id/name/state/group) auto-drops `column_values` from the GraphQL query, ~3x faster per page.
