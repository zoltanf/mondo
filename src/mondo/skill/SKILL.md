...
- **Project before format** with `-q JMESPATH` (applied before the formatter). For the "give me id, name, status" case, `--fields KEY1,KEY2,...` (CSV of keys; dotted paths walk nested dicts) is shorter than the equivalent `-q` projection. Both are global flags surfaced in the "Output / Query" help panel. JMESPath functions are strict about null: `length(x)` and `sort_by(...)` exit 2 when the field is null on any row — guard with `[?field]` or ``length(x || `[]`)``.
- **Computed columns (mirror/formula/board_relation/dependency) are not empty.** monday returns `text: null` for these types by design — the rendered value is in `display_value` on the same column_values entry, and typed reads (`item list/get`, `subitem list/get`, `item find`) also fill `text` from it so naive `.text` consumers work. A null `text` on a mirror column is **not** a bug and **not** a reason to escape to `mondo graphql`.
...
- **Write discipline: only mutate what the user asked for.** Don't assign people, flip statuses, post updates, or create items beyond the explicit request — unrequested writes are the top source of permission denials in agent sessions (and have landed on the wrong item). If the user asked to review a draft first, show the update/doc body and wait for approval before posting. Read-backs after your own writes are fine.
- **Parse with `-q`/`--fields`/`jq`, not inline Python.** Piping mondo output into a `python3 -c` one-liner is the top self-inflicted failure in agent sessions — escaped-quote f-strings die with `SyntaxError`, and `None` values break format specs. A `-q '<jmespath>'` projection or a `jq` filter covers almost every reshaping need; when real scripting is unavoidable, write a small `.py` file and run that instead of inlining.
- **Never `2>/dev/null` a mondo call.** Errors, recovery hints, and the JSON error envelope live on stderr; suppressed, a failure is just empty stdout + a nonzero exit. Benign cache-hit notices are withheld in non-TTY runs (the stale-skill warning still appears, at most once per day), so stderr is near-errors-only in pipelines — use `2>&1` or leave it attached and branch on the exit code.
...
- **Empty a column with `mondo column clear`** — it sends the type-correct empty payload. Guessed empties through `column set` (`--value ""`, `'{"labels":[]}'`) fail codec validation; the error now points you at `column clear` explicitly.
- **Shell-quote values.** Any `--value`, `--column k=v`, or `--name` containing `&`, spaces, or parens must be quoted — an unquoted `&` forks the shell mid-command (`command not found: _Rights` is this failure).
- **Cleanup:** delete commands soft-archive by default; pass `--hard` for true delete.
- **Escape hatch:** `mondo graphql '<query>'` for anything no subcommand wraps. It emits the **unwrapped `data` object** by default (so `-q`/jq address the payload directly — no `.data` prefix); pass `--raw` for the full `{data, errors, extensions}` envelope. **Pass the query as a positional** — `-q/--query` is the global JMESPath projection, not the GraphQL query. If a GraphQL document lands in `--query` anyway, mondo runs it as the query (stderr note) but disables the projection for that call; pass it positionally to combine with `-q`. Before reaching for `graphql`, check there isn't already a subcommand: a file/asset download link is `mondo file url --asset <id> -q '[0].public_url'`, **not** a hand-written `graphql 'query { assets(ids:[…]) { public_url } }'`. Typed reads cover the computed columns natively — `item list/get`, `column get`, and exports render mirror/formula/board_relation/dependency `display_value`, fill `text` from it, and carry `linked_item_ids` for relation types — so the old `... on MirrorValue { display_value }` inline-fragment escape is never needed for reads (see `references/columns.md`).
