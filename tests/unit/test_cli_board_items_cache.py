...
class TestBareListCache:
    def test_cold_list_writes_cache(self, httpx_mock: HTTPXMock, cache_file: Path) -> None:
        _warm(httpx_mock)
    ...
    def test_second_list_serves_from_cache(self, httpx_mock: HTTPXMock, cache_file: Path) -> None:
        _warm(httpx_mock)
        result = runner.invoke(app, ["item", "list", "--board", BOARD])
        assert result.exit_code == 0, result.output
    ...
    def test_group_variant_served_from_cached_full_list(self, httpx_mock: HTTPXMock) -> None:
        _warm(httpx_mock)
    ...
    def test_max_items_served_truncated_from_cache(self, httpx_mock: HTTPXMock) -> None:
        _warm(httpx_mock)
        result = runner.invoke(app, ["item", "list", "--board", BOARD, "--max-items", "1"])
        assert result.exit_code == 0, result.output
        assert [r["id"] for r in json.loads(result.stdout)] == ["1"]
        assert len(httpx_mock.get_requests()) == 1

    def test_fields_projection_served_from_cache(self, httpx_mock: HTTPXMock) -> None:
        _warm(httpx_mock)
        result = runner.invoke(app, ["--fields", "id,name", "item", "list", "--board", BOARD])
        assert result.exit_code == 0, result.output
...
class TestNonBareVariantsStayLive:
    def test_group_only_cold_does_not_write(self, httpx_mock: HTTPXMock, cache_file: Path) -> None:
        ...
        httpx_mock.add_response(url=ENDPOINT, method="POST", json=_ok(_items_page([ITEMS[1]])))
        result = runner.invoke(app, ["item", "list", "--board", BOARD, "--group", "g2"])
        assert result.exit_code == 0, result.output
        assert not cache_file.exists()

    def test_max_items_cold_does_not_write(self, httpx_mock: HTTPXMock, cache_file: Path) -> None:
        httpx_mock.add_response(url=ENDPOINT, method="POST", json=_ok(_items_page(ITEMS)))
        result = runner.invoke(app, ["item", "list", "--board", BOARD, "--max-items", "1"])
        assert result.exit_code == 0, result.output
        assert not cache_file.exists()

    def test_slim_fields_cold_does_not_write(self, httpx_mock: HTTPXMock, cache_file: Path) -> None:
        httpx_mock.add_response(url=ENDPOINT, method="POST", json=_ok(_items_page(ITEMS)))
        result = runner.invoke(app, ["--fields", "id,name", "item", "list", "--board", BOARD])
        assert result.exit_code == 0, result.output
        assert not cache_file.exists()

    def test_filtered_list_bypasses_cache(self, httpx_mock: HTTPXMock, cache_file: Path) -> None:
        _warm(httpx_mock)
        ...
        httpx_mock.add_response(url=ENDPOINT, method="POST", json=_ok(_items_page([ITEMS[0]])))
        result = runner.invoke(app, ["item", "list", "--board", BOARD, "--filter", "text7=x"])
        assert result.exit_code == 0, result.output
        assert len(httpx_mock.get_requests()) == 3

    def test_group_with_comma_bypasses_cache(self, httpx_mock: HTTPXMock) -> None:
        ...
        httpx_mock.add_response(url=ENDPOINT, method="POST", json=_ok(_items_page(ITEMS)))
        result = runner.invoke(app, ["item", "list", "--board", BOARD, "--group", "g1,g2"])
        assert result.exit_code == 0, result.output
        assert [r["id"] for r in json.loads(result.stdout)] == ["1", "2"]
        assert len(httpx_mock.get_requests()) == 3

    def test_columns_selection_bypasses_cache(self, httpx_mock: HTTPXMock) -> None:
        _warm(httpx_mock)
        httpx_mock.add_response(url=ENDPOINT, method="POST", json=_ok(_items_page(ITEMS)))
        result = runner.invoke(app, ["item", "list", "--board", BOARD, "--columns", "status"])
        assert result.exit_code == 0, result.output
...
class TestCacheFlags:
    ...
    def test_refresh_cache_rewrites(self, httpx_mock: HTTPXMock, cache_file: Path) -> None:
        _warm(httpx_mock)
        fresh = [dict(ITEMS[0], name="A-renamed"), ITEMS[1]]
        httpx_mock.add_response(url=ENDPOINT, method="POST", json=_ok(_items_page(fresh)))
        result = runner.invoke(app, ["item", "list", "--board", BOARD, "--refresh-cache"])
        assert result.exit_code == 0, result.output
        assert len(httpx_mock.get_requests()) == 2
        envelope = json.loads(cache_file.read_text())
        assert envelope["entries"][0]["name"] == "A-renamed"

    def test_expired_envelope_fetches_live(self, httpx_mock: HTTPXMock, cache_file: Path) -> None:
        _warm(httpx_mock)
        envelope = json.loads(cache_file.read_text())
...
class TestWritePathInvalidation:
    def test_item_create_drops_board_cache(self, httpx_mock: HTTPXMock, cache_file: Path) -> None:
        _warm(httpx_mock)
        assert cache_file.exists()
        httpx_mock.add_response(
            url=ENDPOINT,
        ...
        result = runner.invoke(app, ["item", "create", "--board", BOARD, "--name", "New"])
        assert result.exit_code == 0, result.output
        assert not cache_file.exists()

    def test_item_rename_drops_board_cache(self, httpx_mock: HTTPXMock, cache_file: Path) -> None:
        _warm(httpx_mock)
        httpx_mock.add_response(
            url=ENDPOINT,
            method="POST",
        ...
        result = runner.invoke(app, ["item", "rename", "1", "--board", BOARD, "--name", "Z"])
        assert result.exit_code == 0, result.output
        assert not cache_file.exists()

    def test_column_set_drops_board_cache(self, httpx_mock: HTTPXMock, cache_file: Path) -> None:
        _warm(httpx_mock)
    ...
    def test_group_delete_drops_board_cache(self, httpx_mock: HTTPXMock, cache_file: Path) -> None:
        _warm(httpx_mock)
        assert cache_file.exists()
        httpx_mock.add_response(
            url=ENDPOINT,
    ...
    def test_column_delete_drops_board_cache(self, httpx_mock: HTTPXMock, cache_file: Path) -> None:
        _warm(httpx_mock)
        assert cache_file.exists()
        httpx_mock.add_response(
            url=ENDPOINT,
